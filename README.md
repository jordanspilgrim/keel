...
python -m pytest tests/ -q      # 105 tests (policy, guardrails, enforcement, ledger/envelope, live session, money-demo, server)
...
All six phases are built and verified, then hardened through **six** independent review passes (see [`BUILD.md`](BUILD.md) → the *Independent review* sections). The core safety architecture:
...
The judge treats the graded conversation as data, not instructions (a golden fixture embeds a "give all 5s" attack and is still scored fail); the golden set carries per-dimension human scores with judge calibration gated at mean error ≤ 1 point; the seeded red-team is **14 probes** (6 jailbreak incl. paraphrases + 4 off-scope + 4 PII incl. name/address); economics/export formulas and a real two-thread resolve race have regression tests. **105 offline tests, ~1s, no network.**

**The money demo** (`python run_demo.py`) selects the treated segment from the baseline analytics signal (it lands on the price-sensitive segment, where the discount lever helps) and measures the lift there. Conversations are LLM-driven, so numbers vary run to run — I report only the single committed run, whose manifest is in `dashboard/manifests/`. The committed run (`run-20260722T180438`, treated cohort **n=20**) moved the price-sensitive segment **15% → 60% (+45pp)**, margin-adjusted **+35.8pp**, with the overall cohort +33pp as context, at **91.7% eval pass / 100% coverage**. *(That manifest was produced before this sixth review's output-contract and eval-spec changes, so re-run `python run_demo.py` to regenerate it under the current code — the number will move.)* The demo requires a matched paired cohort from an identical starting-state hash, a lever-compatible signal loaded by id, and a strictly-positive treated-segment lift to declare success; each run writes full provenance to `dashboard/manifest.json` plus a dated copy. Guardrail catch rate 100% (14/14 seeded probes), 100% AI-disclosure coverage.
