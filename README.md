...
**The money demo** (`python run_demo.py`) selects the treated segment from the baseline analytics signal (it lands on the price-sensitive segment, where the discount lever helps) and measures the lift there. Conversations are LLM-driven, so numbers vary run to run — I report only the single committed run, whose manifest is in `dashboard/manifests/`. The committed run (`run-20260723T043723`, treated cohort **n=20**, under the current server-authored-output + hashed-eval-spec code) moved the price-sensitive segment **55% → 75% (+20pp)**, margin-adjusted **+15.7pp**; the overall cohort moved **40% → 53% (+13pp)**, at **82% eval pass / 100% coverage** — above the 80% kill-switch floor, so the program stays in normal mode — with a **0.006 fairness gap**. The demo requires a matched paired cohort from an identical starting-state hash, a lever-compatible signal loaded by id, and a strictly-positive treated-segment lift to declare success; each run writes full provenance to `dashboard/manifest.json` plus a dated copy. Guardrail catch rate 100% (14/14 seeded probes), 100% AI-disclosure coverage.
