...
python -m pytest tests/ -q      # 135 tests (policy, guardrails, enforcement, ledger/envelope, live session, money-demo, server)
...
- **Dataset size** — 200 synthetic customers by default (tests/economics); the money demo seeds a larger world (`DEMO_SYNTH_N`) so the treated cohort can be **n=60** for a statistically stable read.
...
All six phases are built and verified, then hardened through **eight** independent review passes (see [`BUILD.md`](BUILD.md) → the *Independent review* sections). The core safety architecture:
...
The judge treats the graded conversation as data, not instructions (a golden fixture embeds a "give all 5s" attack and is still scored fail); the golden set carries per-dimension human scores with judge calibration gated at mean error ≤ 1 point; the seeded red-team is **14 probes** (6 jailbreak incl. paraphrases + 4 off-scope + 4 PII incl. name/address); economics/export formulas and a real two-thread resolve race have regression tests. **135 offline tests, ~1s, no network.**

**The money demo** (`python run_demo.py`) runs the closed loop end-to-end under a paired, snapshot-controlled harness: baseline → cluster the graded conversations → a structured intervention signal selects the highest-loss lever-compatible segment (it lands on price-sensitive) → enable the discount lever + a lead-with-discount playbook → re-measure the **same** cohort from a byte-identical *restored world* (`starting_state_sha` recorded per arm, so eligibility is held constant). The committed run (`run-20260723T190123`, treated cohort **n=60**) moved the price-sensitive segment **15% → 47% (+32pp)**, margin-adjusted **+25pp**; the overall cohort moved **18% → 42% (+25pp)**, at **85% eval pass / 100% coverage** (program healthy), fairness gap **0.045**. **Honest scope:** two variables change together (discount policy + playbook) and there is no randomized holdout, so this is a paired *demonstration* of the flywheel on the treated segment — not an isolated causal estimate (the manifest says so). The cohort is **n=60** for a reason: at n=20 a single customer was 5pp and the one-run lift was noise-dominated (it swung from −10pp to +35pp across runs); n=60 (one customer ≈ 1.7pp) gives a stable, reproducible read. The demo requires a matched paired cohort from an identical starting-state hash, a lever-compatible signal loaded by id, and a strictly-positive treated-segment lift to declare success; each run writes full provenance to `dashboard/manifest.json` plus a dated copy. Guardrail catch rate 100% (14/14 seeded probes), 100% AI-disclosure coverage.
