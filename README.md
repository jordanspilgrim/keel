...
python -m pytest tests/ -q      # 69 tests (policy, guardrails, enforcement, ledger/envelope, live session, server)
...
All six phases are built and verified, then hardened through **three** independent review passes (see [`BUILD.md`](BUILD.md) → *Independent review & remediation*, *Second independent review*, and *Third independent review*). The third pass drove the current architecture: a single typed **offer ledger** (`authorized → presented → accepted`, one active offer) is the source of truth for outcome, cooldown, economics, and the eval envelope; the agent's final turn is a **structured response contract** validated deterministically against that ledger (not regex-policed prose); and batch and live grade from one **persisted eval envelope**, so the judge sees identical, exact evidence on both paths. The flywheel demo (`python run_demo.py`) SELECTS the treated segment from the baseline analytics signal (highest loss impact — it lands on the price-sensitive segment because that's where the discount lever helps) and produces a real, measured lift there. Conversations are LLM-driven and the treated segment is small (n≈8), so the baseline draw — and therefore the lift magnitude — vary run to run: the treated segment's save rate lifts materially (roughly **+10 to +40pp** across observed runs; representative run: **38% → 50%, +12pp**), landing in the ~50–75% range after the act. The **overall** cohort figure is noisier context that mixes in untreated customers and can even move negative on a given draw — the treated segment is the headline because it's where the act applies. Each run writes full provenance to `dashboard/manifest.json`, and dated copies are retained in `dashboard/manifests/` so these ranges are backed by artifacts, not prose. The demo requires a matched paired cohort, a lever-compatible signal, and a strictly-positive treated-segment lift to declare success. Eval pass rate is a strict, trace-aware judge (varies ~89–94% by run); guardrail catch rate 100% (11/11 seeded probes), 100% compliance coverage.
